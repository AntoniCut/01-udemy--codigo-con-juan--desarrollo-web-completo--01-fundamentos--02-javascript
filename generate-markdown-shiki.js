...
const deriveSource = (htmlUrlPath) => {
    ...
    //*  -----  Derivar el path de un script por convención de sección  -----
    //*  -----  Patrón: 0X-seccion/archivo-js.html → src/scripts/0X-seccion/archivo.js  -----
    //*  -----  (los markdown son UNO POR CADA archivo .js de la página)  -----
    const SECTION_JS_RE = /^(\d{2}-[^/]+)\/((?:\d{2}-)?[a-z0-9-]+)-js\.html$/;

    if (SECTION_JS_RE.test(relHtml)) {

        /** - `Ruta relativa del archivo fuente JavaScript de la sección` */
...
export const generateMarkdownShiki = async () => {
    ...
    /** - Contiene los archivos de ruta (acepta 'route-XX.js' y 'XX-route-YY.js') */
    const routeFiles = readdirSync(routesDir).filter(
        f => (f.startsWith('route-') || /^\d{2}-route-/.test(f)) && f.endsWith('.js') && f !== 'route-manifest.js'
    );
